...
const material = new THREE.ShaderMaterial({
  ...
  uniforms: {
    ...
    uFrequency: { value: new THREE.Vector2(6.0, 3.5) },
    uTime: { value: 0.0 },
...
camera.position.set(0.0, 0.23, 0.7);
scene.add(camera);
...
const animate = () => {
  ...
  //カメラを円周上に周回させる
  //円の方程式
  camera.position.x = Math.sin(elapsedTime * 0.1) * 3.0;
  camera.position.z = Math.cos(elapsedTime * 0.1) * 3.0;

  camera.lookAt(
    Math.cos(elapsedTime), Math.sin(elapsedTime) * 0.5, Math.sin(elapsedTime) * 0.4
  );
